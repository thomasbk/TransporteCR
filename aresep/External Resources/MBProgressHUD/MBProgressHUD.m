...
- (void)layoutSubviews {
	...
	//CGSize labelSize = [label.text sizeWithFont:label.font]; // tbk deprecated
    CGSize maximumLabelSize = CGSizeMake(label.frame.size.width, CGFLOAT_MAX);
    
    CGRect labelSize = [label.text boundingRectWithSize:maximumLabelSize
                                                   options:NSStringDrawingUsesLineFragmentOrigin| NSStringDrawingUsesFontLeading
                                                attributes:@{NSFontAttributeName:label.font}
                                                   context:nil];
    
	labelSize.size.width = MIN(labelSize.size.width, maxWidth);
	totalSize.width = MAX(totalSize.width, labelSize.size.width);
	totalSize.height += labelSize.size.height;
	if (labelSize.size.height > 0.f && indicatorF.size.height > 0.f) {
    ...
    CGSize maximumDetailLabelSize = CGSizeMake(maxWidth, CGFLOAT_MAX);
    
    CGRect detailsLabelSize = [detailsLabel.text boundingRectWithSize:maximumDetailLabelSize
                                                   options:NSStringDrawingUsesLineFragmentOrigin| NSStringDrawingUsesFontLeading
	...
	if (detailsLabelSize.size.height > 0.f && (indicatorF.size.height > 0.f || labelSize.size.height > 0.f)) {
		totalSize.height += kPadding;
	}
	...
	labelF.origin.x = roundf((bounds.size.width - labelSize.size.width) / 2) + xPos;
	labelF.size = labelSize.size;
	label.frame = labelF;
