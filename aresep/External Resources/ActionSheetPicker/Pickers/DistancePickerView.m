...
- (void)didMoveToWindow {
    ...
    for (int component=0; component<self.numberOfComponents; component++) {
        ...
        if (text) {
            ...
            NSString *longestString = [labels objectForKey:keyName];
            CGRect frame;
            //frame.size = [longestString sizeWithFont:labelfont]; //TBK deprecated
            CGSize maximumLabelSize = CGSizeMake(self.frame.size.width, CGFLOAT_MAX);
            
            CGRect labelSize = [longestString boundingRectWithSize:maximumLabelSize
                                                                       options:NSStringDrawingUsesLineFragmentOrigin| NSStringDrawingUsesFontLeading
                                                                    attributes:@{NSFontAttributeName:labelfont}
                                                                       context:nil];
            frame.size = labelSize.size;
